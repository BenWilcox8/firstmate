...
While the away-posture record exists, a merge still needs explicit captain merge authority or yolo per `AGENTS.md` section 7, plus green checks at its live head; the away record alone never grants merge authority, and a merge without that authority holds for the return.
...
The alarm does not replace reliable injection into an idle supported composer.
