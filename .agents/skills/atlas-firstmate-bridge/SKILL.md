...
- Do not add `--repo` or `--by` to a brief's Atlas commands.
  `bin/fm-spawn.sh` launches every ship and scout worker with `ATLAS_REPO` and `ATLAS_AXI_BY`, so a bare `atlas-axi` reaches this home's map and each write is attributed to that task.
