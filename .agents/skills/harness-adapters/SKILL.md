...
description: >-
  ...
  Contains verified facts for claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, gemini, muse, omp, and agy.
...
Deliver lifecycle actions only through `../../../bin/fm-control.sh <task-id> interrupt|exit|relaunch|park|resume`.
...
Rovo dispatch is disabled for every task kind, so do not select it for launch or recovery.
