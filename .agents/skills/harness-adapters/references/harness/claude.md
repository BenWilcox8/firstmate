...
Every unpinned Claude spawn therefore pre-registers the directory its pane starts in before launch, and the dialog does not appear: the task worktree for a ship or scout, and the home itself for a `--secondmate` spawn, in either seeded shape (a leased worktree or a standalone clone).
...
Explicit account pins retain their trust-dialog procedure without a trust grant in the supervisor store.
