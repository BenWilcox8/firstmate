...
- `STARTUP_MEMORY_BUDGET: startup memory <N> estimated tokens exceeds <M> token budget (<breakdown>)` - the combined size of the startup memory files (captain.md, captain-shared.md, learnings.md) exceeds this home's configured budget.
  Run `/stow` to curate and reduce those files until the next session start is silent.
...
- `SECONDMATE_LIVENESS: secondmate <id>: previous endpoint <target> was not retired: <reason>` - the relaunch succeeded, but the endpoint it replaced could not be closed or proven gone (`docs/agent-control.md` "Endpoint retirement").
  The secondmate is live; close the named leftover pane yourself before trusting the next pane audit.
