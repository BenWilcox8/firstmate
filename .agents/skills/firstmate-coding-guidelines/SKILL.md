...
A skill that belongs to an optional module declares its trigger in the block that the module emits at session start, not in section 13, so a home that does not enable the module never reads it.
