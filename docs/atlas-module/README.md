...
| `bin/fm-atlas-module.sh` | Entry point for the prompt fragments and the dispatch warning | Called by the core hook points below |
| `bin/fm-atlas-hook.sh` | Ticket lifecycle calls: start, complete, land, abort | Called by spawn, the two merge paths, and cleanup |
...
- `bin/fm-spawn.sh`: the `--ticket` flag, the `atlas_ticket=` record line, the dispatch warning call, the crewmate fragment call, and the lifecycle start call.
- `bin/fm-pr-merge.sh`, `bin/fm-merge-local.sh`, and `bin/fm-teardown.sh`: the lifecycle close-out calls.
