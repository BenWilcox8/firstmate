...
A finding, decision, blocker, failure, or completed-work outcome must not depend on the mate model remembering to append a line.
The parent channel therefore provides a script-owned delivery path for durable facts, while the mate's chat remains the place for judgement.
...
- No orphan lifecycle: teardown refuses to stop a child with an undelivered outcome, the same way it refuses on other unlanded conditions.
  After the endpoint stops, a refusal would leave the pool slot without an owner, so an undelivered final line keeps its pending record and the ledger pass retries it; a newer line replaces an older owed line (`tests/fm-local-cleanup-slot.test.sh`).
