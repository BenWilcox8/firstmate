...
Actionable wakes include captain-relevant status signals, no-verb signals without positive evidence that their crew is still executing, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers on their opening recheck or once they remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
One declared wait costs the watcher at most one recheck per `FM_PAUSE_RESURFACE_SECS` window: every later sighting of the same declaration inside that window is absorbed and logged, whether the pane repainted, the watcher re-armed, or the worker reached another turn boundary, and a new status line is a new declaration that earns its own.
...
Away mode is the exception throughout, because the daemon owns triage there and is handed the plain undecorated wake identity to classify itself.
...
A recheck repeating that declaration within the re-surface window is absorbed into the triage log instead of queued, and rebuilds the cleared throttle so the timed path cannot deliver it one poll later; a pane repaint or another turn boundary on the same unchanging declaration earns nothing more, and only a new status append or a full window since the real delivery earns a fresh one.
...
`bin/fm-backend.sh` centralizes backend selection, `state/<id>.meta` helpers, metadata-only cleanup identity validation, selector resolution, and operation dispatch; `bin/backends/tmux.sh` is the verified reference adapter ([`docs/tmux-backend.md`](tmux-backend.md)), `bin/backends/herdr.sh` (P2) has its own required CI lane ([`docs/herdr-backend.md`](herdr-backend.md)), and `bin/backends/zellij.sh` (P3) and `bin/backends/cmux.sh` (P5) remain experimental task-spawn adapters with no dedicated real-backend CI lane.
...
Runtime auto-detection is innermost-first: `$TMUX` wins over `HERDR_ENV=1`, which wins over cmux's primary `CMUX_WORKSPACE_ID` marker and documented fallback signals; auto-detected herdr or cmux prints a one-time opt-out notice, auto-detected tmux stays silent, and zellij is never auto-detected.
Orca is unsupported for new tasks and remains known only for existing-record handling.
...
The dormant Orca adapter retains existing worktree and terminal lifecycle records, including `orca_worktree_id=` and `terminal=`, and removes worktrees through `orca worktree rm` only after the usual firstmate teardown checks pass.
[`orca-backend.md`](orca-backend.md) owns current behavior and limitations, while [`verification/runtime-backends.md`](verification/runtime-backends.md#orca) owns current verification evidence.
...
Crewmates never intentionally touch your project clone; [treehouse](https://github.com/kunchenguid/treehouse) pools clean worktrees for tmux, herdr, zellij, and cmux tasks, while existing Orca records retain their separately managed worktrees.
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, gemini, muse, and omp while preserving the requested profile for later audit.
...
Both `bin/fm-pr-merge.sh` and `bin/fm-merge-local.sh` enforce the AGENTS.md merge-authority contract at the tool boundary: on entry each script reads `yolo=` from the task's `state/<id>.meta` and refuses with a diagnostic naming the task id and the value found when the field is `off` or absent (the safe default).
Pass `--captain-authorized` (before `--` on `fm-pr-merge.sh`, as the second argument on `fm-merge-local.sh`) to carry an explicit current captain merge instruction through the guard; that flag is never forwarded to the forge CLI.
...
Fleet state lives in each task's session-provider backend (tmux by hard default, herdr or cmux when selected or auto-detected, zellij when explicitly selected), no-mistakes run records, status event logs, local markdown under `data/` including `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md`, and persistent secondmate homes.
