...
Herdr's container shape is workspace-per-home plus tab-per-task: the primary home uses workspace label `firstmate`, secondmate homes use `2ndmate-<secondmate-id>` (or a persisted `.fm-herdr-workspace-label` override written by `--workspace-label`), and list-live recovery keys on the meta-recorded `herdr_tab_id=` in each home's own metas (no longer an `fm-` label-prefix scan), scoped to the current `FM_HOME`'s workspace.
