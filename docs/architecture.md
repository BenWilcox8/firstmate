...
Its optional default-off presentation projection is orthogonal to that: it may place one clean new task in a disposable workspace of its own, without consulting the slot ledger and without changing endpoint authority or lifecycle ownership; [Optional presentation spaces](herdr-backend.md#optional-presentation-spaces) owns that conditional design and its narrow home-local restored-shell cleanup at locked session start.
