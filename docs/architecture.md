...
The fleet snapshot and Bearings paths use the concurrent remote-ledger collection, cache, mixed-fleet fallback, and remote-liveness boundary owned by `bin/fm-fleet-snapshot.sh`'s header.
...
The default path concurrently collects registered remote-home ledgers under one shared bound and may refresh their parent-side cache; live GitHub enrichment exists only behind the bearings `--include-prs` opt-in.
...
Stalled escalation delivery writes `state/.subsuper-inject-wedged`, attempts a configured backend-independent active alert, and hands the digest to the captain's durable inbox after `FM_MAX_DEFER_SECS` instead of silently deferring forever; [`wedge-alarm.md`](wedge-alarm.md) owns the delivery ladder.
...
Pane placement inside that default container (tab vs split, the slot plan, overflow, and husk reaping) is owned by `agent-axi`, not firstmate bash: the herdr adapter delegates the pane lifecycle to it and keeps only a minimal fallback for hosts without agent-axi, and firstmate wires agent-axi's `layout --repair` / `snapshot` into session start and the watcher heartbeat (`docs/herdr-backend.md` "Delegation architecture").
...
For Herdr, [`herdr-backend.md`](herdr-backend.md#native-fallback-contract) owns the process-aware boundary for replacing a restored task tab.
