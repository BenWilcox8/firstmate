...
The pause re-surface throttle is cleared wherever a window's pause bookkeeping resets, so each delivered recheck is also recorded against the completed turn and status signature that produced it.
A recheck repeating both within the re-surface window is absorbed into the triage log instead of queued, and rebuilds the cleared throttle so the timed path cannot deliver it one poll later; a new turn, a new status append, or a full window since the real delivery earns a fresh one.
