...
An opt-in per-home `config/herdr-layout=split` alternative instead splits crewmate/scout panes into the spawning firstmate's own pane and tab, falling back to tab-per-task when there is no split anchor or the crewmate-pane cap is reached; secondmate spawns always keep tab-per-task regardless of the toggle (`docs/herdr-backend.md` "Pane-split layout mode").
