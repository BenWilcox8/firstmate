...
Pane placement inside that default container (tab vs split, the slot plan, overflow, and husk reaping) is owned by `agent-axi`, not firstmate bash: the herdr adapter delegates the pane lifecycle to it and keeps only a minimal one-tab-per-task fallback for hosts without agent-axi, and firstmate wires agent-axi's `layout --repair` / `snapshot` into session start and the watcher heartbeat (`docs/herdr-backend.md` "Pane layout is owned by agent-axi").
Its optional default-off presentation projection is orthogonal to that: it may place one clean new task in a disposable workspace of its own, without consulting the slot ledger and without changing endpoint authority or lifecycle ownership; [`docs/herdr-backend.md`](herdr-backend.md#optional-disposable-single-task-presentation-spaces) owns that conditional design.
