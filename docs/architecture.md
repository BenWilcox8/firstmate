...
A declared wait never surfaces as a bare stale pane, however many fresh pane hashes its harness footer produces; endpoint liveness only chooses how that cadence opens.
...
The pause re-surface throttle is cleared wherever a window's pause bookkeeping resets, so that opening recheck is also recorded against the completed turn it was delivered in: an identical recheck for the same declaration in the same turn is absorbed into the triage log instead of queued, and a new turn or a new status append earns a fresh one.
