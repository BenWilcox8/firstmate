...
Actionable wakes include captain-relevant status signals, no-verb signals without positive execution evidence, authenticated check output, stale panes, bounded wait rechecks, and heartbeat backstop hits.
A provably working stale pane can defer escalation for its worker's declared wait or current worktree writes.
An armed `config/wedge-defer-parked-gate` also permits deferral for that worker's unanswered supervisor decision at a validation gate.
Declared waits and attended captain-held transfers use `FM_PAUSE_RESURFACE_SECS`.
One unchanged declaration earns at most one recheck per window, despite pane repaints, watcher re-arms, or new turn boundaries.
A new status append starts a new declaration.
...
Runtime auto-detection is innermost-first: `$TMUX` wins over `HERDR_ENV=1`, which wins over cmux's primary `CMUX_WORKSPACE_ID` marker and documented fallback signals; auto-detected Herdr stays silent like tmux, while auto-detected cmux prints a one-time notice because cmux remains experimental, and zellij is never auto-detected (only explicit selection).
...
That keeps spawn launch compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, gemini, muse, omp, and agy while preserving the requested profile for later audit.
...
After the forge accepts firstmate's merge request, the merge path records away or attended authority against the task's canonical PR identity.
The away record selects the execution posture after the merge-authority gate described above passes.
Its presence alone never grants merge authority.
