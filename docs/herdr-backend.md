...
- **Session start (`bin/fm-bootstrap.sh`, `herdr_layout_repair_sweep`).** At a locked session boundary, the full `agent-axi layout --repair` converges the live workspace to the slot plan (reaps husks, re-binds drifted labels), so a layout that drifted while firstmate was away self-heals on session start. A converged workspace stays silent; a heal reports one `BOOTSTRAP_INFO:` fact. It is one of the seven mutating sweeps skipped in the lock-refused read-only path.
...
A session with a positively stopped server also becomes `missing`.
...
The native busy verdict also checks the process view.
A stale `working` registration cannot make a shell-only pane busy.
`tests/fm-backend-herdr-agent-exit-shell-e2e.test.sh` covers the live-Pi versus leftover-shell distinction.
The [verification record](verification/runtime-backends.md#agent-lifecycle-control) owns versioned evidence.
