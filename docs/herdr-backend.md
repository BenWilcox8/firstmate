...
Watching and attaching: each firstmate home gets its own herdr workspace (the primary uses `firstmate`; each secondmate uses `2ndmate-<secondmate-id>`, or a human-readable override supplied via `bin/fm-spawn.sh`'s `--workspace-label`), with one tab per task inside it.
Tab display labels default to `fm-<id>` but can be set to free-form human-readable text (e.g. `crew: fix login`) via `bin/fm-spawn.sh`'s `--label`; task identity is the meta-recorded tab id, not the label.
...
Verify it works by spawning a trivial task with `--backend herdr` and confirming the task's meta records `backend=herdr` plus `herdr_session=`, `herdr_workspace_id=`, `herdr_tab_id=`, and `herdr_pane_id=`; the workspace for your home should show the new task tab.
...
### Workspace-label derivation (stable, derived from the home itself)

`fm_backend_herdr_workspace_label` (`bin/backends/herdr.sh`) resolves the WORKSPACE label from `$FM_HOME`, read fresh on every call rather than cached or threaded through env plumbing:

- A persisted `.fm-herdr-workspace-label` at the home's root wins, if present: it holds a human-readable DISPLAY label firstmate supplies once via `bin/fm-spawn.sh`'s `--workspace-label` (e.g. `2nd: onestopgreek`). The first non-blank line is used, trimmed; an all-blank file falls through to the derived label rather than yielding an empty one.
- Otherwise the PRIMARY home (no `.fm-secondmate-home` marker at its root) resolves to the constant `firstmate` - byte-identical to every pre-P3 task's recorded label.
- Otherwise a SECONDMATE home (carrying `.fm-secondmate-home`, written by `bin/fm-home-seed.sh` at seed time and containing exactly that secondmate's id) resolves to `2ndmate-<secondmate-id>`, e.g. `2ndmate-sshhip-h7`.

Because every source is the home's own durable identity - a marker file at the home's root, not an environment variable passed down a call chain - the label is automatically stable across every respawn, recovery, and firstmate restart for the life of that home, with no extra bookkeeping required.
That stability is exactly why the persisted override can safely BE the workspace's display label: `fm_backend_herdr_workspace_find` and `_ensure` both resolve through this same function, so find and create always agree on the label, whether it is derived or overridden.
Two different secondmate homes get two different, non-colliding derived labels because their marker ids are unique (verified: `tests/fm-backend-herdr.test.sh`'s `test_workspace_label_different_secondmates_get_different_labels`); when firstmate supplies free-form `--workspace-label` text instead, it is responsible for keeping those distinct, exactly like the pre-existing label-collision adoption semantics below.

Every workspace-scoped adapter path reads this SAME resolution: find/ensure (`fm_backend_herdr_workspace_find`/`_ensure`), tab create (`fm_backend_herdr_create_task`), list-live recovery (`fm_backend_herdr_list_live`), and pane-for-tab (`fm_backend_herdr_pane_for_tab`, via the workspace id these resolve).
So a secondmate's own recovery calls are automatically scoped to its own space and never see (or collide with) the primary's or a sibling secondmate's tabs.

### Human-readable TAB labels and the identity split

Task TAB labels are free-form human-readable DISPLAY text, supplied by firstmate at spawn time via `bin/fm-spawn.sh`'s `--label` (e.g. `crew: fix login` for a crewmate tab, `secondmate: onestopgreek` for a secondmate's launch tab).
When `--label` is absent the tab label defaults to `fm-<id>`, byte-identical to pre-knob behavior.
Both knobs are herdr-only display text: tmux window names stay `fm-<id>`, and every non-herdr backend ignores them (verified: `tests/fm-backend.test.sh`'s `test_spawn_label_flags_are_tmux_noop`).

Because labels are now free-form, they can no longer carry task IDENTITY: two unrelated tasks may legitimately share a label, and herdr enforces no label uniqueness anyway.
Identity lives entirely in each task's `state/<id>.meta` (`herdr_session=`/`herdr_workspace_id=`/`herdr_tab_id=`/`herdr_pane_id=`), and every matching operation keys on those recorded ids, never on a label prefix or an exact label match:

- **Duplicate/husk detection** (`fm_backend_herdr_create_task`): on a respawn, `bin/fm-spawn.sh` reads this task's own recorded `herdr_tab_id=` from its existing meta and passes it as the 5th `<prior_tab_id>` argument. create_task reconciles exactly that tab - a still-live (or ambiguous) prior tab refuses the respawn, a confirmed husk (dead or no-agent) is closed and replaced, and an absent or gone prior tab just creates. A fresh spawn (empty prior tab id) never scans the tab list and never refuses on a label match.
- **Recovery / list-live** (`fm_backend_herdr_list_live`): iterates THIS home's own herdr task metas (`backend=herdr`, matching `herdr_session=`), and for each whose recorded `herdr_tab_id=` is still present in this home's workspace, emits `<session>:<pane_id>\t<label>` with the tab's current live display label. It no longer scans tab labels for an `fm-` prefix.

The meta-recorded tab id is a reliable identity key because herdr preserves workspace/tab/pane ids exactly across an ordinary server restart within the same named session ("ID stability across a server restart" below), which is the realistic recovery scenario.
...
This workspace-level adoption is still label-based (on the stable derived-or-persisted workspace label); TAB-level duplicate/husk detection is NOT - it keys on the meta-recorded tab id (see "Human-readable TAB labels and the identity split" above), so two tabs freely sharing a display label never confuse it.
...
New WORKSPACE lookup does not adopt old secondmate labels: for new spawns and recovery, the adapter matches the current workspace label resolved from `FM_HOME` (`2ndmate-<secondmate-id>`, or a persisted `.fm-herdr-workspace-label` override).
If an older live workspace is still labeled `firstmate-<secondmate-id>`, rename it with `herdr workspace rename <workspace_id> 2ndmate-<secondmate-id>` before expecting new tasks or recovery to use that workspace.
Task-tab recovery (`list-live`) is separately keyed on the meta-recorded tab id, so it is likewise unaffected by whatever display label a tab carries.
...
Each home's own workspace (`firstmate` for the primary, `2ndmate-<secondmate-id>` for a secondmate, or a persisted `.fm-herdr-workspace-label` override - see "Workspace-label derivation" above) is created once per session and reused by every subsequent spawn from that home: `fm_backend_herdr_workspace_ensure` calls `fm_backend_herdr_workspace_find` first and creates a workspace only when none labelled for that home exists yet.
...
A workspace whose label this adapter did not resolve for the active home (see "Workspace-label derivation" above) is never adopted, reused, or torn down by firstmate - `fm_backend_herdr_workspace_find` only ever matches a home's own resolved workspace label, and `fm_backend_herdr_list_live` only ever looks inside that home's own workspace (then keys on the home's own meta-recorded tab ids).
...
| Duplicate/husk check | `herdr tab list --workspace <id>`, match by the meta-recorded `<prior_tab_id>` | Keyed on the task's own recorded tab id (`bin/fm-spawn.sh` passes it on respawn), never the label. Labels are free-form display text and herdr enforces no label uniqueness, so a label match cannot identify a task; the recorded id can. |
...
| Recovery / list-live | iterate this home's `state/<id>.meta` (`backend=herdr`, matching `herdr_session=`); for each recorded `herdr_tab_id=` still present in `herdr tab list --workspace <id>`, emit it | Meta-id driven: the display label is free-form so an `fm-` prefix filter no longer identifies a task tab. The recorded tab id is a reliable key across an ordinary server restart (see "ID stability" below). `<id>` is always THIS home's own workspace (`fm_backend_herdr_workspace_find`), so recovery never sees a sibling home's tabs. |
...
Practical consequence: a stored `herdr_pane_id=`/`herdr_tab_id=` remains a valid, fast-path operational target across an ordinary server restart within the same named session, regardless of how many other homes' workspaces coexist in that session.
This id stability is precisely what lets `fm_backend_herdr_list_live` do meta-id-driven recovery (match each home's recorded `herdr_tab_id=` against the live workspace) rather than the old `fm-` label-prefix scan, now that display labels are free-form.
In a differently-configured or freshly-created session the old ids simply are not present, so list-live lists nothing there - the same read-only, no-op-when-absent behavior it always had.

## Respawn idempotency: a restored task tab is a husk, not a duplicate

A restart's other consequence (the previous section's "what does NOT survive") used to make every fleet respawn after it a manual chore: a restored task tab comes back alive but with a fresh shell process and no registered agent (`agent_status` reset to unknown, `agent get` reporting `agent_not_found`) - or, if the pane's own process failed to restart at all, structurally gone (`pane get` reporting `pane_not_found`).
Before this fix, `fm_backend_herdr_create_task`'s duplicate guard treated either shape identically to a genuinely live duplicate and refused unconditionally, so recovering a fleet after a real herdr server restart (or, worse, a full reboot) meant closing every husk pane by hand before firstmate could spawn into it again - this reproduced in production on 2026-07-03.

The guard is now husk-aware, and keyed on the task's meta-recorded prior tab id (the 5th `<prior_tab_id>` argument `bin/fm-spawn.sh` reads from the respawning task's existing meta), not a label match.
`fm_backend_herdr_pane_agent_state` classifies the recorded prior tab's pane as one of `dead` (`pane get` -> `pane_not_found`), `no-agent` (the pane exists but `agent get` -> `agent_not_found` - the restored-plain-shell shape, and also what a future `resume_agents_on_restore = false` herdr config would produce unconditionally), `live` (a real registered `agent_status`, including idle/blocked - never just "working"), or `unknown` (anything unparseable or unexpected).
Only `dead` and `no-agent` are treated as a husk; `live` and `unknown` both refuse, fail-safe toward refusal whenever the state cannot be classified with confidence.
A confirmed husk is closed and replaced instead of refused: `fm_backend_herdr_create_task` always creates the REPLACEMENT tab first, closes the recorded husk tab by id only after that succeeds, and verifies that husk tab id is gone before returning success.
...
4. `fm_backend_herdr_list_live`, called with `FM_HOME` set to each home in turn, sees only that home's own tab(s): the primary's list shows only `cm1`; the secondmate's list shows only `cm2` (not `e2esm1`, whose meta lives in the primary's state since the primary spawned it), and neither list leaks into the other.
