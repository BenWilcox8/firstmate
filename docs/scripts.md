...
| `fm-browser-reap.sh`      | Stop headless browser chains whose owning agent session is gone, never one whose owner is alive |
...
| `fm-brief-blocks-lib.sh` | One owner of the scaffold blocks shared across brief kinds: the status states line, a crewmate brief's rules 4-7, and the subagent model-tier block |
...
| `backends/orca.sh`       | Dormant Orca adapter for existing worktree and terminal records                  |
...
| `fm-pane-hash-lib.sh`    | Single owner of the `state/.hash-<key>` marker derivation: footer normalization, volatile-digit collapse, and pane hash computation |
...
| `fm-atlas-module.sh`     | Print the optional Atlas module's supervisor block, a ticketed worker's crewmate block, and the ticket-less dispatch warning, only in a wired home, plus the worker pane's Atlas environment lines and the names the filtered launch environment passes through |
...
| `fm-atlas-word-lib.sh`   | Parse the `--captain-word` option that the merge and cleanup scripts pass to the Atlas close-out |
