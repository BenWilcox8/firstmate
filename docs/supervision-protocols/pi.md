...
Every other routine outcome returns as an appended, rendered note that leads with ⛵ then the dim outcome text, unless the [routine supervision notes preference](../configuration.md#routine-supervision-notes-preference-configroutine-supervision-notes) hides it.
