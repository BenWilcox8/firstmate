...
`pi` is a `#!/usr/bin/env node` script, so an earlier record (2026-07-07, made without a real `pi` to test - a long-lived `node -e` proxy was used instead, since `pi --version` exited too quickly under the same pane to capture a live foreground state) assumed a live `pi` pane would report the generic `node` as its `pane_current_command` and left `pi` unclassifiable.
...
So `fm_backend_tmux_agent_state` now classifies an exact `pi` foreground command as `alive` (exact match, so an unrelated `pip`/`mpi*` command is never caught).
The match is additive and monotonic-safe: an older or differently-packaged `pi` that still surfaces as a bare `node` falls through to `ambiguous` (unchanged), a bare shell still reports `dead`, and the secondmate-liveness sweep relaunches on `dead`/`missing` only - so there is no false-respawn risk, only a positive-liveness gain for the current default harness.
Since `node` is also the generic name for a plain interpreter session, any future JS-based harness, or someone's unrelated node script, there is still no way to attribute a bare `node` foreground process back to `pi` from outside the pane without deeper (and fragile) argument introspection.
The classifier therefore still reports `ambiguous` for a bare `node`/`python`/`python3` rather than guess - per the sweep's correctness bar, a wrong `alive` is harmless but a wrong `dead` spins up a duplicate agent, so an unresolvable interpreter name must never be treated as confidently dead.
Practical effect: a node-wrapped Pi secondmate pane is never auto-healed, preserving duplicate prevention.
...
The agent-liveness probe positively classifies all five verified harnesses on the current default builds; only an older or differently-packaged `pi` that surfaces as a bare `node` process falls back to the safe `ambiguous` verdict (see the `pi` liveness note above), and authoritatively absent Pi windows are covered by `missing`.
