...
In split mode, crewmate/scout panes are split into the spawning firstmate's own pane (spawner on the left, crewmates as side-by-side full-height columns of near-equal width in the right half - each spawn splits the widest column and then rebalances) instead of getting their own tab; `FM_HERDR_SPLIT_MAX` (default 3) caps the columns before further spawns overflow to tab mode, and the layout falls back to tab mode when firstmate is not itself running in a herdr pane.
...
FM_HERDR_SPLIT_MAX=3   # herdr-only: max crewmate columns beside the spawner in split mode before further spawns overflow to tab mode
FM_HERDR_SPLIT_RATIO=0.5  # herdr-only: ratio for the split-mode pane split call itself; the post-spawn rebalance then evens all columns out
FM_BACKEND_HERDR_SPLIT_REBALANCE_MAX=24  # herdr-only: iteration cap for the best-effort post-spawn column rebalance (docs/herdr-backend.md "Pane geometry")
