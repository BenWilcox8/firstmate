...
# Recovery additionally requires the exact stable process proof below.
        ...
        foreground_processes: ([.foreground_processes[] | {
          ...
          agent: ((.name | base) as $name
            ...
            | (($name | test("^(claude|codex|opencode|grok)"))
              or ($name | test("^pi(-signed)?$|^kimi(-code)?$|^muse(-bin-.+)?$|^cursor-agent$|^agy$|^omp$"))
...
# Echoes empty|pending|unknown|target-missing|send-failed, a subset of the proof-carrying
...
# endpoint are different facts, and they used to reach the caller wearing the
# same word. Ask herdr's own structured refusal once before calling a submit
# merely unknown, so fm-send can name a gone pane instead of inviting a resend
# against an endpoint that no longer exists.
fm_backend_herdr_unknown_or_missing() {
  ...
  while :; do
    ...
    if [ "$baseline" = idle ]; then
      ...
      case "$verdict" in
        ...
        unknown) fm_backend_herdr_unknown_or_missing; return 0 ;;
      esac
