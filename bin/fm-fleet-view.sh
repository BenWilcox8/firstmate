...
printf '%s\n' "$SNAPSHOT" | jq -r '
  ...
  def endpoint_of($t):
    if $t.parked != null then "parked (closed on purpose)"
  ...
  def action_of($t):
    if $t.parked != null then "waits on: \($t.parked.reason) - resume with bin/fm-control.sh \($t.id) resume"
    elif $t.kind == "secondmate" then "\($t.actions.send) - \($t.actions.watch)"
