...
# bounds that home's startup-memory curation, primary config/agent-limit
# gives the whole fleet one concurrent agent limit (bin/fm-agent-limit-lib.sh),
# and primary
...
FM_INHERITABLE_CONFIG="${FM_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode lavish-axi-host agent-limit}"
