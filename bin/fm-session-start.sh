...
#                       (same-home backlog reconciliation, secondmate
#                       convergence, secondmate liveness, pending remote handoff
#                       retry, herdr layout repair, X-mode artifact writes,
...
#             liveness, pending remote handoff retry, Herdr layout repair, and
...
if [ "$READ_ONLY" -eq 1 ]; then
  BOOT_OUT=$(FM_BOOTSTRAP_DETECT_ONLY=1 FM_BOOTSTRAP_NETWORK=skip \
    FM_BOOTSTRAP_VERBOSE_FACTS="$VERBOSE" \
  ...
  BOOT_OUT=$(
    ...
    FM_BOOTSTRAP_NETWORK=skip FM_BOOTSTRAP_VERBOSE_FACTS="$VERBOSE" \
