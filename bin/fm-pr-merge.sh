...
# <words>, before the optional -- separator, with the captain's exact words from
# chat to record them as the Atlas approval before the ticket is completed; it
# is never forwarded to the forge CLI, and the hook's header owns a refused
# close-out.
#
...
# Usage: fm-pr-merge.sh <task-id> <pr-url> [--captain-authorized] [--captain-word <words>]
