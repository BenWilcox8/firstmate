...
Use \`$PAUSED_VERB [at=<epoch>]: {why}\` only for a known external wait that can clear on its own.
...
WORKER_RULES=$(fm_brief_worker_rules ship "$PAUSED_VERB" "$STATUS_FILE" "$ASK_USER_BLOCK") || exit 1
