...
#   then tmux.
#   Spawn-capable backends are tmux, verified herdr, and experimental zellij
#   and cmux. Orca is unsupported for new tasks and remains for existing records.
#   Auto-detected herdr and tmux stay silent; cmux prints a stderr notice.
...
#   /updatefirstmate, restart). A bare adapter name (claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|omp|agy)
...
# hooks are written, every unpinned claude launch pre-registers the directory the pane
...
# On success prints: spawned <id> harness=<name> [account=<alias>] kind=<ship|scout|secondmate> [mode=<mode> yolo=<on|off>] window=<backend-target> worktree=<path>
...
launch_template() {
  ...
  claude)
    ...
    printf '%s' '__MODELFLAG____EFFORTFLAG____NAMEFLAG__"$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
    ;;
  ...
  # Rovo remains dormant because the dispatch boundary refuses every launch.
...
# below takes effect without it. Every unpinned Claude launch needs it, a secondmate's
...
case "$HARNESS" in
claude*)
  # Account pins select another store. Never grant trust in the supervisor store.
