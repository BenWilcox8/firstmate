...
case "$BACKEND" in
  ...
  herdr)
    ...
    # Pane placement (tab vs split, overflow, slot fill order) is owned by
    # agent-axi's durable slot ledger, invoked through the create_task delegation
    # in bin/backends/herdr.sh (spec agent-axi/v1; docs/herdr-backend.md
    # "Delegation architecture"). fm-spawn only ensures the home's workspace and
    # asks create_task for the task's pane; the old config/herdr-layout split
    # branch was deleted in phase 1. Without agent-axi, create_task falls back to
    # one plain tab per task (docs/herdr-backend.md "Native fallback contract").
    ...
    # character; the seeded tab id is threaded through to create_task untouched,
    # which is the only function permitted to prune it (never re-derived from
