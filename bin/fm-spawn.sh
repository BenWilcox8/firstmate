...
#   receives the Atlas environment: `export ATLAS_AXI_BY=<holder>`, where
#   <holder> follows the fm-<task-id> rule of `fm-atlas-hook.sh start`, so every
#   atlas-axi write the worker makes is attributed to its task; and, only when
#   `fm-atlas-hook.sh wired` resolves this home's config/specs pointer,
#   `export ATLAS_REPO=<that repo>`, so a bare atlas-axi reaches the map.
...
#   marker FM_TASK_ID and the Atlas names ATLAS_AXI_BY and ATLAS_REPO that ship
...
if [ "$KIND" = ship ] || [ "$KIND" = scout ]; then
  ...
  # The same worker reaches the Atlas under its own author name: ATLAS_AXI_BY
  # follows the fm-<task-id> holder rule of `fm-atlas-hook.sh start`, and
  # ATLAS_REPO is this home's resolved config/specs pointer, sent only when the
  # home is wired.
  case "$ID" in
