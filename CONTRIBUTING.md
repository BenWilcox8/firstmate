...
Require the actual job contexts: `Lint 1` through `Lint 4`, `Test coverage guard`, `Repo invariants`, `Stock macOS Bash snapshot compatibility`, `Behavior portable parallel 1`, `Behavior portable parallel 2`, `Behavior portable serial 1` through `Behavior portable serial 9`, `Behavior tests (Herdr)`, `Behavior timing aggregate`, and `PR must be raised via no-mistakes`.
...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; spawn-supported values are `tmux`, `herdr` (which has its own required CI lane), `zellij`, and `cmux`, with the latter two remaining experimental and having no dedicated real-backend CI lane; Orca is unsupported for new tasks and retains only existing-record handling, while `codex-app` is documented only in `docs/codex-app-backend.md`.
...
```sh
while IFS= read -r script; do bash -n "$script" || exit; done < <(bin/fm-lint.sh --list-files)   # syntax-check the shell surface fm-lint.sh will cover (changed files locally, full set in CI/on main)
...
A test that pins a restricted `PATH`, or that names a tool by absolute path in a fixture, must go through `fm_test_core_path` and `fm_test_tool` in `tests/lib.sh`: `/usr/bin` and `/bin` hold almost nothing on a host that keeps its tools elsewhere, so a hardcoded `/bin/ps` or a bare FHS `PATH` is a path to nothing there.
