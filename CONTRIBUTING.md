...
tests/fm-backend-herdr.test.sh            # fake herdr CLI unit tests for the experimental herdr adapter, including version/tool gates, target parsing, send/capture, native agent-state submit confirmation, ANSI-aware structural composer-state guard/fallback coverage with bordered, unbordered, and faint Codex suggestion fixtures, slash-submit retry regression coverage, native busy state, per-home workspace-label resolution, default-tab prune safety, restored-layout husk replacement, verified CLI bug workarounds, and opt-in pane-split layout mode (layout/cap/ratio resolution, spawner-right and column-right placement, cap overflow and override, both no-anchor fall-back paths, pane-label husk close-and-replace, live-duplicate refusal, and cross-layout tab/pane duplicate-husk detection)
