...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse` and `gemini` for crewmates and scouts only; Rovo dispatch is disabled; never dispatch on an unverified or disabled adapter.
...
Keep the task description, acceptance criteria, constraints, exclusions, and necessary context explicit across those subsections.
