...
state/               runtime records and signals; gitignored
  ...
  .hash-* .count-* .stale-* .stale-since-* .churn-since-* .paused-* .wedge-escalations-* .dead-reported-* .writing-* .waiting-* .seen-* .hb-surfaced-* .herdr-layout-drift-surfaced .last-* .heartbeat-streak   watcher internals; never touch
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse`, `gemini`, and `agy` for crewmates and scouts only; Rovo dispatch is disabled; never dispatch on an unverified or disabled adapter.
...
Also load it when a live worker reports its no-mistakes pipeline dead, unreachable, or timed out; section 13 owns the full trigger.
...
Copy the URL from the worker's ready status, the task's `pr=` metadata, or the forge response, never from memory.
For review and merge asks, repeat that URL in MAIN's final captain-facing response.
...
Keep accepted requirements, constraints, exclusions, and necessary context explicit across those subsections.
...
The skill also owns the guarded fleet restart procedure.
