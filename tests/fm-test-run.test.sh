...
test_family_selection() {
  ...
  listed=$("$RUNNER" --list --family watcher-wake-lock)
  printf '%s\n' "$listed" | grep -Fxq 'tests/fm-daemon-codex-short-wake.test.sh' \
    || fail "watcher family must include durable short-wake regression"
  listed=$("$RUNNER" --list --family live-harness-optin)
  printf '%s\n' "$listed" | grep -Fxq 'tests/fm-backend-herdr-codex-submit-e2e.test.sh' \
    || fail "live harness family must include the Codex submit regression"
...
test_coverage_is_locale_independent() {
  local candidate selected='' out rc
  ...
  if [ -z "$selected" ]; then
    printf 'skip: no non-C locale available for coverage comparison\n'
    return 0
  fi
  rc=0
  out=$(LC_ALL="$selected" "$RUNNER" --check-coverage 2>&1) || rc=$?
  [ "$rc" -eq 0 ] || fail "coverage must pass under $selected: $out"
  case "$out" in *'not in sorted order'*) fail "coverage compared C-sorted input under $selected: $out" ;; esac
  pass "coverage remains complete under a non-C locale"
}
