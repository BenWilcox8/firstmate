...
test_pi_threads_model_and_max_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "FM_PI_HARNESS=pi '$FAKEBIN_DIR/pi' --tui-mode regular --model 'openai-codex/gpt-5.6-sol' --thinking 'max' --name '$id' -e" \
    "pi launch did not force the regular TUI while threading the requested model, max thinking level, and native name"
...
test_pi_signed_threads_shared_pi_profile_and_preserves_identity() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "FM_PI_HARNESS=pi-signed '$FAKEBIN_DIR/pi-signed' --tui-mode regular --model 'openai-codex/gpt-5.6-sol' --thinking 'max' --name '$id' -e" \
    "pi-signed launch did not force the regular TUI with Pi's model, thinking, native-name, and extension semantics"
  assert_contains "$launch" "fm-operational-input.sh' encode launch-brief" \
...
test_pi_signed_persistent_secondmate_uses_pi_extensions_and_identity() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "FM_PI_HARNESS=pi-signed '$FAKEBIN_DIR/pi-signed' --tui-mode regular --name 'Secondmate, $id' -e '$sm/.pi/extensions/fm-primary-turnend-guard.ts' -e '$sm/.pi/extensions/fm-primary-pi-watch.ts'" \
    "pi-signed secondmate did not force the regular TUI with Pi's native name and primary extension launch shape"
