...
#!/usr/bin/env bash
set -u
if [ "${FM_FAKE_HERDR_HUSK:-0}" = 1 ] || [ "${FM_FAKE_HERDR_PROCESS:-agent}" = shell ]; then
  ...
  pane)
    case "${2:-}" in
      ...
      get)
        ...
        exit 0 ;;
      process-info)
        if [ "${FM_FAKE_HERDR_PROCESS:-agent}" = unreadable ]; then
          exit 1
        elif [ "${FM_FAKE_HERDR_HUSK:-0}" = 1 ] || [ "${FM_FAKE_HERDR_PROCESS:-agent}" = shell ]; then
...
exit 0
SH
  chmod +x "$fb/no-mistakes" "$fb/gh" "$fb/gh-axi" "$fb/glab" "$fb/tmux" "$fb/ps" "$fb/herdr"
  printf '%s\n' "$fb"
}
