#!/usr/bin/env bash
# fm-native-session-lib.sh: proving a running worker's native session identity.
#
...
. "$ROOT/bin/fm-native-session-lib.sh"
...
# that runs <shell-snippet> and then blocks. Sets STAND_IN_PID and records it
# for the EXIT trap, so call it directly and never in a command substitution.
...
# ns_report <status>: print the library's result globals, or its refusal.
ns_report() {
  if [ "$1" -ne 0 ]; then
...
capture() {  # <harness> <worktree> <state> <id> <gen> <pid>...: echoes the result, returns its code
  fm_native_session_capture "$@"
  ns_report "$?"
}
...
test_claude_capture_proves_the_running_session() {
  ...
  stand_in claude "CLAUDE_CONFIG_DIR=$cfg" ':'; pid=$STAND_IN_PID
  claude_record "$cfg" "$pid" "$SID_A" "$wt"
  transcript=$(claude_transcript "$cfg" "$SID_A")
  out=$(capture claude "$wt" '' '' '' "$pid"); rc=$?
...
test_claude_capture_reads_the_default_config_from_the_process_home() {
  ...
  stand_in claude "HOME=$home" ':'; pid=$STAND_IN_PID
  claude_record "$home/.claude" "$pid" "$SID_A" "$wt"
  transcript=$(claude_transcript "$home/.claude" "$SID_A")
  out=$(HOME=/nonexistent capture claude "$wt" '' '' '' "$pid"); rc=$?
...
test_unverified_harness_refuses() {
  ...
  stand_in grok - ':'; pid=$STAND_IN_PID
  out=$(capture grok "$wt" '' '' '' "$pid"); rc=$?
...
test_codex_capture_proves_the_open_rollout() {
  ...
  stand_in codex - "exec 3>>'$rollout' 4>'$lock'"; pid=$STAND_IN_PID
  out=$(capture codex "$wt" '' '' '' "$pid"); rc=$?
...
test_codex_capture_refuses_what_it_cannot_prove() {
  ...
  lock=$(codex_lock "$home" "$SID_A")
  stand_in codex - "exec 4>'$lock'"; pid=$STAND_IN_PID
  out=$(capture codex "$wt" '' '' '' "$pid"); rc=$?
  ...
  stand_in codex - "exec 3>>'$rollout' 5>>'$rollout_b'"; pid=$STAND_IN_PID
  out=$(capture codex "$wt" '' '' '' "$pid"); rc=$?
  ...
  stand_in codex - "exec 3>>'$rollout' 4>'$lock_b'"; pid=$STAND_IN_PID
  out=$(capture codex "$wt" '' '' '' "$pid"); rc=$?
  ...
  stand_in codex - "exec 3>>'$rollout'"; pid=$STAND_IN_PID
  out=$(capture codex "$wt" '' '' '' "$pid"); rc=$?
...
test_pi_capture_proves_the_extension_record() {
  ...
  for harness in pi pi-signed; do
    ...
    stand_in pi - ':'; pid=$STAND_IN_PID
    out=$(capture "$harness" "$wt" "$state" t1 gen-7 "$pid"); rc=$?
...
test_pi_capture_refuses_what_it_cannot_prove() {
  ...
  out=$(capture pi "$wt" "$state" t1 gen-7 "$pid"); rc=$?
...
locate() {  # <harness> <session-id> <file> <worktree> [<claude-config>]
  fm_native_session_locate "$@"
  ns_report "$?"
}
...
test_locate_confirms_a_resumable_session() {
  ...
  transcript=$(claude_transcript "$cfg" "$SID_A")
  out=$(locate claude "$SID_A" "$transcript" "$wt" "$cfg"); rc=$?
...
test_locate_refuses_a_session_that_is_gone() {
  ...
  out=$(locate claude not-a-uuid "$transcript" "$wt"); rc=$?
...
test_pi_worker_extension_records_its_session() {
  ...
  out=$(capture pi "$wt" "$state" "$id" "$gen" "$$"); rc=$?
...
resume_launch() {
  fm_native_session_resume_launch "$@" 2>&1
}
