...
1. First action: create your branch: `git checkout -b fm/paused-verb-ship`
...
4. Report status by appending one line:
   `echo "{state}: {one short line}" >> '%FM_HOME%/state/paused-verb-ship.status'`
...
Firstmate steers you through durable message files in '%FM_HOME%/state/paused-verb-ship.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '%FM_HOME%/state/paused-verb-ship.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '%FM_HOME%/state/paused-verb-ship.inbox'/NNN.msg '%FM_HOME%/state/paused-verb-ship.inbox'/handled/`.
