...
run_watcher_bounded() {
  ...
  shift 2
  # 25s, not the tighter 10s this used to carry: a busy CI runner sharing its
  # box with other shards can starve this real fork/exec/poll cycle past a
  # 10s wall clock with nothing actually wedged, which reads here as a bare
  # 124 timeout with no stderr to explain it. The watcher's own bounds
  # (FM_CHECK_TIMEOUT, FM_POLL) are unchanged; only the outer safety net moves.
  perl -e 'my $pid=fork; die unless defined $pid; if (!$pid) { exec @ARGV } local $SIG{ALRM}=sub { kill "TERM", $pid; waitpid $pid, 0; exit 124 }; alarm 25; waitpid $pid, 0; alarm 0; exit($? >> 8)' \
