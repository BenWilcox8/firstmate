...
# shellcheck source=/dev/null # the library is its own lint root; following it here exceeds CI runner memory
...
test_undelivered_records_are_scan_immutable() {
  (
    ...
    # shellcheck disable=SC2329 # the sourced pending-reply library invokes this override
...
test_restart_preserves_expectation_and_parent_destination() {
  ...
  # shellcheck source=/dev/null # the library is its own lint root
  . "$ROOT/bin/fm-pending-reply-lib.sh"
...
test_fm_send_marked_secondmate_creates_pending_and_embeds_corr() {
  ...
  # shellcheck disable=SC2031 # set by the sourced pending-reply library, which lint follows as its own root
  case "$got" in
