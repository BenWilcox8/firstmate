...
EOF
  ...
  fill_brief_subsections "$home/data/$id/brief.md" "$words" 'Preserve this accepted build constraint.'
  out=$(run_spawn "$home" "$fakebin" "$id" "$proj" claude --mode no-mistakes --yolo off)
  assert_present "$home/data/$id/launch-brief.md" "plain intent was not serialized"
  authorized=$(awk '$0 == "## Accepted task requirements for --intent" { emit=1; next } emit && /^Resolve referenced reports/ { exit } emit { print }' "$home/data/$id/launch-brief.md")
  assert_contains "$authorized" "$words" 'accepted intent changed the request text'
  assert_contains "$authorized" 'Preserve this accepted build constraint.' 'accepted intent lost the implementation constraint'
  ...
  authorized=$(awk '$0 == "## Accepted task requirements for --intent" { emit=1; next } emit && /^Resolve referenced reports/ { exit } emit { print }' "$home/data/intent-literal/launch-brief.md")
  assert_contains "$authorized" "$words" 'literal words in the request were scrubbed'
  assert_contains "$authorized" 'Do not paraphrase.' 'the accepted constraint was discarded'
  ...
  for marker in '[captain]' 'Captain:' "Captain's words:" "Captain's ask:" "Captain's intent:"; do
    ...
    id="intent-marked-$n"
    write_brief "$home" "$id" no-mistakes
    printf '# Task\n%s %s\nPreserve this accepted build constraint.\n%s %s\n\n# Definition of done\nDelivery contract: mode=no-mistakes\n' \
    ...
    authorized=$(awk '$0 == "## Accepted task requirements for --intent" { emit=1; next } emit && /^Resolve referenced reports/ { exit } emit { print }' "$home/data/$id/launch-brief.md")
    assert_contains "$authorized" "$marker Keep the original request intact." 'legacy intent changed its original text'
    assert_contains "$authorized" "$marker Preserve its provenance." 'legacy intent lost its second requirement'
    assert_contains "$authorized" 'Preserve this accepted build constraint.' 'legacy intent discarded its constraint'
  done
