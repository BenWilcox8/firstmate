...
# local-only project): no origin bare repo and no publisher clone.
# The primary checkout's own default branch is then advanced past the pooled
# worktree's allocation base, so a refresh has real work to prove.
