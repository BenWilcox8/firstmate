...
# The native fallback (agent-axi absent) classifies a duplicate tab's pane
# conservatively: if the pane is not resolvable (pane list returns nothing),
# it refuses rather than guessing - fail-safe toward refusal. The refusal never
# creates a replacement tab or closes any pane.
test_create_task_refuses_duplicate_label() {
  ...
  # pane list (response 2) returns nothing - unresolvable pane triggers conservative refusal
  printf '{"result":{"panes":[]}}\n' > "$resp/2.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" FM_HERDR_LOG="$log" FM_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 fm-dup1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "create_task should refuse when the duplicate tab's pane is not resolvable"
  assert_contains "$out" "already exists" "create_task did not report the duplicate label"
  assert_not_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create' "the native fallback must not create a replacement tab when refusing"
  assert_not_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''close' "the native fallback must not close any pane when refusing"
  assert_not_contains "$(cat "$log")" $'\x1f''agent'$'\x1f''get' "the native fallback must not call agent get when the pane is not resolvable"
